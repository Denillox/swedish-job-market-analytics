...
swedish-job-market-analytics/
├── src/
│   ├── extract.py              # Extracts job postings from the JobTech API
│   ├── transform.py            # Transforms raw job data with PySpark
│   ├── quality_checks.py       # Runs data quality checks on processed outputs
│   ├── upload_to_blob.py       # Uploads raw, processed, and export data to Azure Blob Storage
│   └── config.py               # Central configuration for paths and extraction patterns
├── data/
│   ├── raw/                    # Raw CSV data generated by the extraction step
│   ├── processed/              # Processed Parquet datasets generated by Spark
│   └── exports/                # Clean CSV exports used by Power BI
├── docs/
│   ├── ReportPage.png
│   ├── ReportPage2.png
│   └── ReportPage3.png
...
├── Dockerfile                  # Docker image definition with Python, Java, and dependencies
├── docker-compose.yml          # Defines pipeline services for extract, transform, quality, upload, and full pipeline
├── requirements.txt
├── .env.example
├── .gitignore
└── README.md
...
```bash
git clone https://github.com/Denillox/swedish-job-market-analytics.git
